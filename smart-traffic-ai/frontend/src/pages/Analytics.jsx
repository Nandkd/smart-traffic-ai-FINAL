...
function MetricBar({ label, value, max = 1, color = '#EF4444' }) {
...
export default function Analytics() {
  ...
  const [heatView, setHeatView] = useState('week') // 'week' | 'day'
  ...
  // Build heatmap matrix: rows = days, cols = hours
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1400px] mx-auto">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 mt-0.5">Traffic patterns, model performance, and predictive insights</p>
      </div>
      ...
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Wait Time Reduction', value: `${summary.avg_wait_reduction_pct}%`, color: 'text-green-400' },
            { label: 'Ambulance Response ↑', value: `+${summary.ambulance_response_improvement_pct}%`, color: 'text-crimson-400' },
            { label: 'Est. Fuel Saved', value: `${summary.fuel_savings_liters_estimated.toLocaleString()}L`, color: 'text-amber-400' },
            { label: 'CO₂ Reduced', value: `${summary.co2_reduction_kg_estimated.toLocaleString()} kg`, color: 'text-emerald-400' },
          ].map((item, i) => (
            <motion.div
              key={item.label}
      ...
      <div className="glass-card p-5">
        ...
        <ResponsiveContainer width="100%" height={200}>
          <AreaChart data={congHistory.map((c, i) => ({
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        ...
        <div className="glass-card p-5">
          <h3 className="font-display font-semibold text-sm text-white mb-5">Vehicle Type Distribution (7 days)</h3>
        ...
        <div className="flex items-center justify-between mb-5">
          ...
          <div className="flex items-center gap-3 text-xs text-slate-500">
            <span className="flex items-center gap-1.5">
              <span className="w-3 h-3 rounded-sm bg-green-500/60" /> Low
            </span>
            <span className="flex items-center gap-1.5">
              <span className="w-3 h-3 rounded-sm bg-amber-500/60" /> Medium
            </span>
            <span className="flex items-center gap-1.5">
              <span className="w-3 h-3 rounded-sm bg-crimson-500/60" /> High
            </span>
          </div>
        </div>
      ...
      {summary?.model_accuracy && (
        <div className="glass-card p-5">
          <h3 className="font-display font-semibold text-sm text-white mb-5">Model Performance Metrics</h3>
          <div className="grid md:grid-cols-3 gap-6">
            <div className="space-y-3">
              <p className="text-xs font-mono text-slate-500 uppercase tracking-wider mb-3">YOLOv8 Detection</p>
            ...
            <div className="space-y-3">
              <p className="text-xs font-mono text-slate-500 uppercase tracking-wider mb-3">CNN Ambulance</p>
            ...
            <div className="space-y-3">
              <p className="text-xs font-mono text-slate-500 uppercase tracking-wider mb-3">Congestion Ensemble</p>
